...
export const ProductCat1 = () => {
  const [tradProducts, setTradProducts] = useState<Product[]>([]);
  ...
  return (
    <>
          ...
          <Box maxWidth="xl" className="text-center">
            ...
            >
              <Link href="/foot" className="flex items-center group">
                ...
                <span className="ml-2 hover:text-yellow-400 transition-colors">
                  collection
                </span>
              </Link>
            </Typography>
      ...
      <Box
        maxWidth="xl"
        className="mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 justify-items-center gap-y-12 gap-x-8 mt-4 mb-14 px-4"
      >
        {loading
          ...
          ? tradProducts.slice(0, 4).map((product, index) => (
              <motion.div
                key={index}
          ...
          : (
            <Box className="col-span-full flex flex-col items-center justify-center py-12 text-gray-400">
              ...
              <Typography variant="h6">No products available right now.</Typography>
            </Box>
          )}
      </Box>
    </>
