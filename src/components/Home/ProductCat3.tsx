...
export const ProductCat3 = () => {
  const [beltsProducts, setBeltsProducts] = useState<Product[]>([]);
  const [beadsProducts, setBeadsProducts] = useState<Product[]>([]);
  ...
  useEffect(() => {
    const fetchAllProducts = async () => {
      try {
        const [beltsRes, beadsRes, capsRes] = await Promise.all([
          fetch("/api/products/belts"),
          fetch("/api/products/beads"),
          fetch("/api/products/caps"),
        ]);
  ...
  return (
    <>
      ...
      <Box className="container">
        <Box className="flex justify-center mb-8 p-4 mt-10">
          <Box maxWidth="xl" className="text-center">
            <Typography
              variant="h2"
              className="gold_title text-gray-50 font-extrabold md:text-7xl sm:text-5xl flex items-center justify-center gap-4"
            >
              <Link href="/accessories" className="flex items-center group">
                <motion.svg
                  ...
                  fill="currentColor"
                  viewBox="0 0 16 16"
                  className="text-gray-50 group-hover:text-yellow-400 transition-colors"
                  animate={{ x: [0, 5, 0] }}
        ...
        {loading
          ...
          : (
            <Box className="col-span-full flex flex-col items-center justify-center py-12 text-gray-400">
              ...
              <Typography variant="h6">No accessories available right now.</Typography>
            </Box>
          )}
      </Box>
    </>
